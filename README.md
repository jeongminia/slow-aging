...
- 조건에 맞는 추천 메뉴를 최대 3개까지 선별하고 최대 20단계 조리법 제공

### 개인 맞춤형 저속노화 식사 조건

- 밥·반찬·국·찌개·후식·일품 중 오늘 원하는 메뉴 유형 선택
- 최대 조리시간을 설정해 조리 부담을 고려한 레시피 탐색
- `단백질 우선`과 `나트륨 낮게` 조건으로 영양 점수 기준 조정
- 통곡물·콩류·채소 구성을 반영하고 초가공식품과 튀김 조리를 감점
- 우선 소진 재료와 제외·알레르기 재료를 반영해 사용자별 후보 구성
- 식품안전나라 원본 레시피에 프로젝트의 설명 가능한 저속노화 기준을 적용
